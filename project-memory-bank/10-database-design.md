...
**Phase 10 schema implemented, 2026-07-18** (`packages/database/prisma/schema.prisma`).

## Current tables

- **`User`** (`users`) — `id` (uuid), `email` (unique), `passwordHash` (argon2, **nullable since Phase 10** — an SSO-only user has no password at all, not a placeholder hash; `login()` rejects password auth when null), `displayName`, `role` (enum, `USER` default), `createdAt`, `updatedAt`, `version`.
- **`Organization`** (`organizations`, Phase 10) — `id` (uuid), `name`, `isPersonal` (boolean, default `false` — `true` only for the invisible workspace auto-created at registration), `createdAt`, `updatedAt`, `version`.
- **`Membership`** (`memberships`, Phase 10) — `id` (uuid), `organizationId` (FK to `Organization`, cascade delete), `userId` (FK to `User`, cascade delete), `role` (enum `MembershipRole`: `OWNER`/`ADMIN`/`MEMBER`, default `MEMBER`), `createdAt`, `updatedAt`, `version`. `@@unique([organizationId, userId])` — one membership row per user per org. Indexed on `userId`.
- **`SsoIdentity`** (`sso_identities`, Phase 10) — `id` (uuid), `userId` (FK to `User`, cascade delete), `provider` (enum `SsoProvider`: `OIDC`/`SAML`), `providerName` (string — which configured connection, e.g. `"oidc"`/`"default"`), `providerUserId` (string — the OIDC `sub` claim or the SAML NameID), `createdAt`, `updatedAt`, `version`. `@@unique([provider, providerName, providerUserId])`. Indexed on `userId`.
...
- **`Project`** (`projects`) — `id` (uuid), `ownerId` (FK to `User`, cascade delete — "creator," used for destructive-permission checks), `organizationId` (**new, Phase 10** — FK to `Organization`, cascade delete, NOT NULL), `name`, `description` (nullable), `status` (enum `ProjectStatus`: `ACTIVE`/`ARCHIVED`, default `ACTIVE`), `archivedAt` (nullable, set when archived), `createdAt`, `updatedAt`, `version`. Indexed on `ownerId`, `organizationId`. `DELETE /projects/:id` sets `status = ARCHIVED` + `archivedAt` rather than removing the row (soft delete); requires the creator or an org `ADMIN`/`OWNER` (Phase 10) — any member can read/update.
- **`Goal`** (`goals`) — `id` (uuid), `projectId` (FK to `Project`, cascade delete), `ownerId` (FK to `User`, cascade delete), `organizationId` (**new, Phase 10** — plain string, denormalized from the parent `Project` at creation time, same rationale as `ownerId`; no Prisma relation declared, since cascade-delete already flows through `Project`→`Organization`), `title`, `description` (nullable), `status` (enum `GoalStatus`: `NOT_STARTED`/`IN_PROGRESS`/`COMPLETED`/`ARCHIVED`, default `NOT_STARTED`), `targetDate` (nullable), `completedAt` (nullable, set when status becomes `COMPLETED`), `createdAt`, `updatedAt`, `version`. Indexed on `projectId`, `ownerId`, `organizationId`. `status`/`completedAt` are driven automatically by `GoalsService.recalculateProgress` based on child `Task` completion (see [08-backend-guidelines.md](08-backend-guidelines.md)) as well as settable manually via `PATCH`.
- **`Task`** (`tasks`) — `id` (uuid), `goalId` (FK to `Goal`, cascade delete), `ownerId` (FK to `User`, cascade delete), `organizationId` (**new, Phase 10** — denormalized from the parent `Goal`, same pattern as `Goal.organizationId`), `title`, `description` (nullable), `status` (enum `TaskStatus`: `TODO`/`IN_PROGRESS`/`DONE`/`ARCHIVED`, default `TODO`), `order` (int, default 0, decomposition ordering within a goal), `completedAt` (nullable, set when status becomes `DONE`), `createdAt`, `updatedAt`, `version`. Indexed on `goalId`, `ownerId`, `organizationId`. `DELETE /tasks/:id` sets `status = ARCHIVED` (soft delete).
...
**Phase 7 additions:** `ExecutionEvent` gained a composite `@@index([ownerId, createdAt])` — the analytics velocity endpoint's query pattern (`WHERE ownerId = ? AND createdAt >= ?`) isn't covered by the existing single-column `ownerId` index alone. No new tables — Phase 7 (`@pee/analytics`) is a pure read layer over Phases 2-6's existing tables, computing every metric via live query aggregation (`groupBy`/`aggregate`/bounded `findMany`) rather than a materialized rollup, per Sustainable Complexity (Principle 8) — see [21-decision-log.md](21-decision-log.md) and [27-backlog.md](27-backlog.md).

**Phase 10 additions:** `Organization`/`Membership`/`SsoIdentity` (new tables); `organizationId` on `Project`/`Goal`/`Task`; `User.passwordHash` made nullable. `Project.organizationId` has a full Prisma relation (needed since `Project` has no other path to `Organization`); `Goal.organizationId`/`Task.organizationId` are plain denormalized `String` columns with no declared Prisma relation — cascade-delete integrity is already guaranteed transitively (`Organization`→`Project`→`Goal`→`Task`, each step `onDelete: Cascade`), so a second direct relation would be redundant, not a correctness gap. `TaskExecutionSession`/`ExecutionEvent`/`AIRecommendation` deliberately did **not** gain `organizationId` — Phase 10 kept execution/AI/analytics owner-scoped (see [08-backend-guidelines.md](08-backend-guidelines.md) and [27-backlog.md](27-backlog.md)).
