...
## Post-Phase-10 — Production Hardening (P0)

**Status: Complete (2026-07-19).** Not a product phase — a full production-hardening audit (`SYSTEM_PROMPT.md` Part 1's hardening-review role, run against the whole repo by four independent verification passes) scored overall readiness **4/10, not approved for production release**, driven by two Critical findings (CI's migration step silently no-op'd so no e2e result was verifiable; there was no deployment target of any kind) plus a P0 list of High findings. All seven P0 items were fixed this session:

| P0 item | What changed |
|---|---|
| CI migration no-op | `packages/database/prisma/migrations/20260719000000_init/` generated via `prisma migrate diff --from-empty` (no live DB needed for a from-empty diff) — `migrate deploy` in CI now has a real schema to apply. Caveat: not yet applied against a live Postgres in this sandbox (still no Docker) — re-verify the first time one is available |
| No deployment target | `infrastructure/docker/api.Dockerfile` + `web.Dockerfile` (multi-stage; `apps/web` gained `output: 'standalone'` in `next.config.js` and — confirmed by inspecting a real build — has zero runtime dependency on any `@pee/*` package since every import is `import type`), `infrastructure/docker/docker-compose.prod.yml`, full runbook in [15-deployment.md](15-deployment.md). Caveat: real `docker build`/`docker run` not executed (no Docker daemon in this sandbox) — every command inside each Dockerfile was verified by running its non-Docker equivalent locally (`next build`, `tsc`, `prisma generate`) |
| No structured logging | New `packages/logging` (`@pee/logging`) — `StructuredLogger` (JSON-line NestJS `LoggerService`), `RequestContext` (`AsyncLocalStorage`-based correlation), `RequestIdMiddleware`, `RequestLoggingInterceptor`, wired into `services/api`'s `AppModule`/`main.ts`. 7 unit tests |
| No client-side request timeouts | `apps/web/lib/fetch-with-timeout.ts` (10s default, 15s for AI calls) adopted across all 6 `apps/web/lib/*-api-client.ts` files with try/catch converting timeouts to each function's existing null/error fallback shape; `apps/desktop`'s `AuthSession` and `apps/mobile`'s `MobileAuthSession` gained the same local `fetchWithTimeout` — since `remote-ipc.ts`/`remote-client.ts` already funnel every execution/AI/analytics call through those two classes' `authedFetch`, this one change covers all of both native clients' online traffic |
| Fake `/health`, no graceful shutdown | `HealthController` now runs a real Prisma `SELECT 1` and returns 503 if unreachable; `main.ts` calls `app.enableShutdownHooks()` so `PrismaService.onModuleDestroy` (already implemented) actually runs on `SIGTERM` |
| No boot-time env validation | `services/api/src/env.validation.ts` — a Joi schema on `ConfigModule.forRoot`, covering `DATABASE_URL`/JWT secrets/AI provider keys (conditionally required by `AI_PROVIDER`)/all `SSO_*` vars (conditionally required by whichever provider is configured) |
| SSO config/secret gaps | `.env.example` now documents all 11 `SSO_*` vars actually referenced in code (previously undocumented); `SamlBridgeService.secret()` no longer falls back to `JWT_ACCESS_SECRET` then `''` — it throws `ServiceUnavailableException` (503) if `SSO_SAML_BRIDGE_SECRET` is unset, with a regression test proving `JWT_ACCESS_SECRET` is never even read |

Also fixed as a direct consequence of the health-controller change: `services/api/test/health.controller.spec.ts` rewritten (real Prisma mock, asserts both the healthy and 503 paths). Test suite grew from 319 to 328 (9 new: 7 in `@pee/logging`, 2 in the rewritten health spec, 1 SAML fail-closed regression — plus the 319 baseline all still passing unmodified). `npm run build`/`typecheck`/`lint`/`test` all clean across the whole workspace after every change. The audit's P1 list (N+1 query, unbatched sync push, missing composite indexes, no dependency-vulnerability scanning, no API versioning decision, `/organizations` pagination inconsistency, thin README) was **not** actioned this session — tracked in [27-backlog.md](27-backlog.md) as "P1 hardening items from the 2026-07-18 production-hardening audit."

## Next phase

Phase 0, 0.5, 1, 2, 3, 4, 5, 6, 7, 8, 9, and 10 are done, and the P0 production-hardening pass above is complete — **Phase 10 was the last product phase defined in the roadmap.** No further product phase is currently scoped; remaining work lives entirely in [27-backlog.md](27-backlog.md) (run every Docker-dependent e2e suite at least once now that a real migration exists; run `apps/mobile`'s Detox e2e on a real device; a real vendor-credentialed AI smoke test; local SQLite file-at-rest encryption on both native clients; SAML SLO/multi-IdP/client-credential validation; org-visibility for execution/AI/analytics; email-token-based invites; the P1 hardening items above) — see [20-known-issues.md](20-known-issues.md).
